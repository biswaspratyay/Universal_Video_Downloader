...
class MainWindow(QMainWindow):
    ...
    def build_left_panel(self):
        ...
        self.thumbnail_label.setMinimumSize(220, 126)
        self.thumbnail_label.setSizePolicy(
        ...
        self.download_thumbnail_button = QPushButton("Download Original Thumbnail")
        self.download_thumbnail_button.setToolTip(
            "Save the source thumbnail at its original dimensions"
        )
        self.download_thumbnail_button.setEnabled(False)
        self.download_thumbnail_button.clicked.connect(self.download_thumbnail)
        thumb_layout.addWidget(self.download_thumbnail_button)

        self.left_layout.addWidget(thumb_group, stretch=3)

        ########################################################
        # Video Information
        ########################################################

        self.video_information_group = QGroupBox("Video Information")
        self.video_information_group.setStyleSheet("""
            QGroupBox {
                font-size: 15px;
                font-weight: 700;
                color: palette(window-text);
        ...
        self.video_information_group.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Preferred,
        )

        info_layout = QFormLayout(self.video_information_group)
        ...
        self.title_label = add_row("Title")
        self.channel_label = add_row("Channel")
        self.duration_label = add_row("Duration")
        self.resolution_label = add_row("Resolution")
        self.fps_label = add_row("FPS")
        self.filesize_label = add_row("File Size")
        self.upload_label = add_row("Upload Date")
        self.views_label = add_row("Views")
        self.type_label = add_row("Type")
        self.live_label = add_row("Live")

        self.left_layout.addWidget(self.video_information_group, stretch=2)

        self.playlist_summary_group = QGroupBox("Playlist Summary")
        playlist_layout = QFormLayout(self.playlist_summary_group)
        ...
        self.playlist_name_value = add_playlist_row("Name")
        self.playlist_uploader_value = add_playlist_row("Uploader")
        self.playlist_videos_value = add_playlist_row("Videos")
        self.playlist_duration_value = add_playlist_row("Total Duration")
        self.playlist_size_value = add_playlist_row("Estimated Size")
        self.playlist_summary_group.setVisible(False)
        self.left_layout.addWidget(self.playlist_summary_group, stretch=1)
        ...
        self.main_layout.addWidget(self.left_scroll_area, 1)
    # --------------------------------------------------------
    # CENTER PANEL
    # --------------------------------------------------------
    ...
    def build_center_panel(self):
        ...
        self.quality_combo.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        self.quality_combo.currentIndexChanged.connect(self.update_format_information)
        self.quality_combo.currentIndexChanged.connect(self._update_resume_button_state)
        self.quality_combo.addItem("Analyze a Video First")

        settings_layout.addWidget(self.quality_combo, row, 1)
        ...
        settings_layout.addWidget(self.format_label, row, 0)

        self.format_combo = QComboBox()
        self.format_combo.addItem("Analyze a Video First")
        self.format_combo.setEnabled(False)
        self.format_combo.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        ...
        self.audio_combo.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)

        self.audio_combo.addItems([
            "Audio Only",
            "Original (Included in video)",
        ])
        self.audio_combo.setCurrentText(self.settings.get("audio_format", "Original (Included in video)"))
        self.audio_combo.currentTextChanged.connect(self._update_format_controls)
        self.audio_combo.currentTextChanged.connect(self._save_download_settings)

        settings_layout.addWidget(self.audio_combo, row, 1)
        ...
        self.folder_input.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)

        self.folder_input.setText(self.settings.get("download_folder", "downloads"))
        self.folder_input.textChanged.connect(self._save_download_settings)

        self._update_format_controls()

        self.browse_button = QPushButton("Browse")
        self.browse_button.setIcon(QIcon(str(FOLDER_ICON)))
        ...
        self.center_layout.addWidget(settings_group)
        ...
        font-weight:bold;
        """)
        self.download_button.setIcon(QIcon(str(DOWNLOAD_ICON)))
        ...
        font-weight:bold;
        """)
        self.resume_button.setIcon(QIcon(str(PLAY_ICON)))
        self.resume_button.setToolTip("No partial download available")
        ...
        self.cancel_button.setIcon(QIcon(str(CANCEL_ICON)))
        self.cancel_button.setEnabled(False)
        self.cancel_button.clicked.connect(self.cancel_download)

        self.resume_hint_label = QLabel("Resume partial download available")
        ...
        button_row.addWidget(self.resume_hint_label)
        button_row.setStretch(0, 2)
        button_row.setStretch(1, 2)
        button_row.setStretch(2, 1)

        self.center_layout.addLayout(button_row)
        self.download_button.clicked.connect(self.download_video)

        ########################################################

        self.main_layout.addWidget(self.center_panel, 2)
    ...
    def _save_download_settings(self):
        self.settings.set("download_folder", self.folder_input.text().strip())
        self.settings.set("video_format", self.format_combo.currentText())
        self.settings.set("audio_format", self.audio_combo.currentText())
        self.settings.save()
        self._update_resume_button_state()

    def _update_format_controls(self):
        audio_only = self.audio_combo.currentText() == "Audio Only"
        self.quality_label.setVisible(not audio_only)
        self.quality_combo.setVisible(not audio_only)
        self.format_label.setVisible(not audio_only)
    ...
    def build_right_panel(self):
        ...
        self.codec_value = add_row("Codec")
        self.container_value = add_row("Container")
        self.audio_value = add_row("Audio")
        self.bitrate_value = add_row("Bitrate")
        self.filesize2_value = add_row("Estimated Size")
    ...
    ########################################################
        ...
        self.history_list.setIconSize(self.history_list.sizeHint())
        ...
        self.main_layout.addWidget(self.right_scroll_area, 1)
    ...
    def build_bottom_panel(self, root):
        ...
        add_stat("Speed", self.speed_label)
        add_stat("Downloaded", self.downloaded_label)
        add_stat("ETA", self.eta_label)
        add_stat("Status", self.status_label)
